...
try:
    ...
    st.write(f"Error in date conversion: {e}")
    st.write("Attempting to parse dates using pd.to_datetime")
    data['visit_created_on'] = pd.to_datetime(data['visit_created_on'], errors='coerce')
...
# Check if filtered data has values before proceeding with calculations
if not filtered_data.empty:
    ...
    # Display date inputs
    with col1:
        date1 = display_date_input(col1, "Start Date", data['visit_created_on'].min(), data['visit_created_on'].min(), data['visit_created_on'].max())

    with col2:
        date2 = display_date_input(col2, "End Date", data['visit_created_on'].max(), data['visit_created_on'].min(), data['visit_created_on'].max())
    ...
    # Filter data based on date inputs
    filtered_data = filtered_data[(filtered_data['visit_created_on'] >= date1) & (filtered_data['visit_created_on'] <= date2)]
    

    # Calculate metrics
    total_visits = filtered_data['visit_id'].nunique()  # Calculate total visits based on unique visit IDs
    ...
    # Correct calculation of average visits
    months = filtered_data['visit_created_on'].dt.to_period('M').nunique()
    average_visits = total_visits / months if months > 0 else 0
    ...
    # Group data by month and count visits
    monthly_visits = filtered_data.groupby(filtered_data['visit_created_on'].dt.to_period('M')).size()
